...
public class MysticVinesFeature extends Feature<TwistingVinesConfig> {

    public MysticVinesFeature(Codec<TwistingVinesConfig> p_67292_) {
        super(p_67292_);
    }
        ...
        for(int i = 1; i <= p_225304_; ++i) {
            if (p_225301_.isEmptyBlock(p_225303_)) {
                if (i == p_225304_ || !p_225301_.isEmptyBlock(p_225303_.above())) {
                    p_225301_.setBlock(p_225303_, CharredBlocks.MYSTIC_VINES.get().defaultBlockState().setValue(GrowingPlantHeadBlock.AGE, Integer.valueOf(Mth.nextInt(p_225302_, p_225305_, p_225306_))), 2);
                    break;
                }

                p_225301_.setBlock(p_225303_, CharredBlocks.MYSTIC_VINES_PLANT.get().defaultBlockState(), 2);
            }
    ...
    private static boolean isInvalidPlacementLocation(LevelAccessor p_67297_, BlockPos p_67298_) {
        if (!p_67297_.isEmptyBlock(p_67298_)) {
            ...
            return !blockstate.is(Blocks.NETHERRACK) && !blockstate.is(CharredBlocks.MYSTIC_NYLIUM.get()) && !blockstate.is(CharredBlocks.MYSTIC_MUSHROOM.get());
        }
    }
}
